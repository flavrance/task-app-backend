namespace TaskApp.Infrastructure.DapperDataAccess.Repositories
{
    ...
    public class CashFlowRepository : ICashFlowReadOnlyRepository, ICashFlowWriteOnlyRepository
    {
        ...
        public async Task Add(CashFlow cashFlow, Credit credit)
        {
            ...
            {
                string insertTasksQL = "INSERT INTO CashFlow (Id, Year) VALUES (@Id, @Year)";
                ...
                int cashFlowRows = await db.ExecuteAsync(insertTasksQL, cashFlowParameters);
        ...
        public async Task<CashFlow> Get(Guid id)
        {
            using (IDbConnection db = new SqlConnection(_connectionString))
            {
                string TasksQL = @"SELECT * FROM CashFlow WHERE Id = @Id";
                Entities.CashFlow cashFlow = await db
                    .QueryFirstOrDefaultAsync<Entities.CashFlow>(TasksQL, new { id });
